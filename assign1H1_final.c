/*
    ...
    Name: Sahaj Agarwal(201601077)
          K.Maheshwari(201601037)
*/
//header files
#include<stdio.h>
#include<time.h>
...
//to swap board position with blank to get all the possible boards 
...
//to check if two boards are same so that we can eliminate duplicates
// we return 1 if boards are same else return -1 
...
//we use removeBoard to remove the board from the open list whenever it has been processed
...
//this function is used to check if the newly generated board is present in closed or openlist
...
//In this function, we swap the blank position in all possible directions and update open and closed lists
//we calculate time and space as well whenever we generate a new node and stores a node respectively
...
//f(n) = g(n) + h(n) where h(n) is the heuristic and g(n) is the number of moves
...
//we use it to check at every point if we have found the solution and return 1 if true else -1
...
//this function has a while loop that runs till the openlist is empty ot till the solution is found
...
//the next three functions check whether the generated board is solvable or not
//if a board is non solvable then we directly print that the board is not solvable and exit the program
...
int main(){
    ...
    //to check if board is solvable or not
        ...
        *space_comp=0;
        ...
        if(finalboard!=NULL){
            ...
            printf("Solved!!!\n\n");
            //printing the time and space required for solving the board
            printf("Moves: %d\n",finalboard->cost);
